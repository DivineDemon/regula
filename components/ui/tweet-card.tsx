  ...
  <div className="flex flex-row items-start justify-between tracking-normal">
    <div className="flex items-center space-x-3">
      <a href={tweet.user.url} target="_blank" rel="noreferrer">
        {/* biome-ignore lint/performance/noImgElement: Twitter profile images are external and dynamic, not suitable for Next.js Image optimization */}
        <img
...
export const TweetBody = ({ tweet }: { tweet: EnrichedTweet }) => (
  <div className="text-[15px] leading-relaxed tracking-normal wrap-break-word">
    {tweet.entities.map((entity, idx) => {
      const key =
        entity.type === "url" || entity.type === "mention"
          ? `${entity.type}-${entity.href}-${idx}`
          : `${entity.type}-${entity.text}-${idx}`;
      switch (entity.type) {
        case "url":
        case "symbol":
        case "hashtag":
        ...
        case "text":
          return (
            <span
              ...
              // biome-ignore lint/security/noDangerouslySetInnerHtml: Tweet content may contain HTML entities that need to be rendered
    ...
    <div className="flex flex-1 items-center justify-center">
      ...
      {tweet.photos && (
        <div className="relative flex transform-gpu snap-x snap-mandatory gap-4 overflow-x-auto">
          ...
          {tweet.photos.map((photo) => (
            /* biome-ignore lint/performance/noImgElement: Twitter photo URLs are external and dynamic, not suitable for Next.js Image optimization */
            <img
              key={photo.url}
              src={photo.url}
              width={photo.width}
      ...
      {!tweet.video &&
        ...
        tweet?.card?.binding_values?.thumbnail_image_large?.image_value.url && (
          /* biome-ignore lint/performance/noImgElement: Twitter card thumbnail URLs are external and dynamic, not suitable for Next.js Image optimization */
          <img
            src={
              // @ts-expect-error package doesn't have type definitions
              tweet.card.binding_values.thumbnail_image_large.image_value.url
